function [all_radii, all_fluxes] = DataImport_estrazioneParam(r_min, r_max)
...
%     s_4 = open('Data001-4.mat'); s_5 = open('Data001-5.mat'); 
    ...
    F(:,1)=s.F1(:,5); F(:,2)=s.F2(:,5); F(:,3)=s.F3(:,5); F(:,4)=s.F4(:,5); 
    F(:,5)=s.F5(:,5); F(:,6)=s.F6(:,5); F(:,7)=s.F7(:,5); F(:,8)=s.F8(:,5); 
    F(:,9)=s.F9(:,5); F(:,10)=s.F10(:,5); F(:,11)=s.F11(:,5); F(:,12)=s.F12(:,5); 
    F(:,13)=s.F13(:,5); F(:,14)=s.F14(:,5); F(:,15)=s.F15(:,5); F(:,16)=s.F16(:,5); 
    F(:,17)=s.F17(:,5);  
  
    % se dal numero minimo di campioni risultassero necessari più di 1500
    % dati...
...
%     Rtot = [R; Rbis; Rtris; R4; R5]; Ftot = [F; Fbis; Ftris; F4; F5];
    ...
    [Rs,index] = sort(R);            % raggi ordinati in ordine crescente
    ...
    all_radii = Rs(50:1450,r_min:r_max);
    all_fluxes = Fs(50:1450,r_min:r_max);
    
    % NOTA: l'eliminazione delle code è da impostare in funzione del numero
    % di dati utilizzato
