...
s = open('DataStem.mat');  % caricamento dei dati
...
BMRi(:,1)=s.F1(:,5); BMRi(:,2)=s.F2(:,5); BMRi(:,3)=s.F3(:,5); BMRi(:,4)=s.F4(:,5);
BMRi(:,5)=s.F5(:,5); BMRi(:,6)=s.F6(:,5); BMRi(:,7)=s.F7(:,5); BMRi(:,8)=s.F8(:,5); 
BMRi(:,9)=s.F9(:,5); BMRi(:,10)=s.F10(:,5); BMRi(:,11)=s.F11(:,5); BMRi(:,12)=s.F12(:,5); 
BMRi(:,13)=s.F13(:,5); BMRi(:,14)=s.F14(:,5); BMRi(:,15)=s.F15(:,5); BMRi(:,16)=s.F16(:,5); 
BMRi(:,17)=s.F17(:,5);
...
NumSam = 150;            % numero di campioni (da impostare volta per volta)
R = R(1:NumSam,:); BMRi = BMRi(1:NumSam,:);   

BMRs = sort(BMRi);       % BMR ordinati in ordine crescente        
BMR = BMRs(15:135,:);    % eliminazione delle code (da impostare volta per volta)
...
results = zeros(15,7);
for Rmin = 9:9           % scorre le finestre
    
    Rmax = Rmin + elements - 1;
    
    % sweep sugli esponenti
    bmin = 1; bstep = 1; bmax = 1; dmin = 0.5; dstep = 0.001; dmax = 1.5;
        ...
        for dInd = 1:length(d)
            ...
            for i = Rmin:Rmax-1        % scorre le coppie di distribuzioni
                for j = i+1:Rmax
                    ...
                    dist = sqrt((log10(BMRcoll(:,i))-log10(BMRcoll(:,j))).^2 + ... 
                           (log10(Fscale(:,i))-log10(Fscale(:,j))).^2);

                    ok = isfinite(dist); NaNind = find(ok == 0);
                    dist(NaNind) = 0;                           % scarto dei punti di non overlapping

                    Nover = Nover + Nbins - length(NaNind);     % somma del numero di punti di overlapping
                    SE(bInd,dInd) = SE(bInd,dInd) + sum(dist);  % somma delle distanze tra le distribuzioni
...
%     perc_avw = mean(wind(:)); perc_stdw = std(wind(:)); perc_medw = median(wind(:));
    ...
    ADvec = ADdata(:); ADind = [ones(121,1); 2*ones(121,1); 3*ones(121,1)]; %4*ones(211,1); 5*ones(21,1)];
